...
def interactive(opt, print_parser=None):
    ...
    if isinstance(opt, ParlaiParser):
        ...
        opt = opt.parse_args()
    opt['task'] = 'parlai.agents.local_human_silent.local_human_silent:LocalHumanAgent'
    ...
    # open json file, load json into some structure
    import json
    all_text = []
    with open('/localtmp/ParlAI/data/qa4patients/clinical_parlai.json', 'r') as f:
        for line in f:
            while True:
                try:
                    jfile = json.loads(line)
                    all_text.append(jfile['text'])
                    break
                except ValueError:
                    line += next(f)
    with open('/localtmp/ParlAI/data/pubmed_qa4p/pubmed_parlai_nonull.json', 'r') as f:
        for line in f:
            while True:
                try:
                    jfile = json.loads(line)
                    ...
                    line += next(f)
    import random
    to_test = random.sample(all_text, min(1000, len(all_text)))
    # randomly sample from json file
    # profile
